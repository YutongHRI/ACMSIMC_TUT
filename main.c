...
void measurement(){
    US_C(0) = CTRL.ual;  //电压给定就是我测量的电压，电压值不区分上一个和下一个值，其专指一个值
    US_C(1) = CTRL.ube;  //观测器里看到的电压就是控制器输出的电压
    US_P(0) = US_C(0);
    US_P(1) = US_C(1);
    ...
    im.omg = IM.x[4];  //im是观测器里的电机模型
}
void inverter_model(){
    IM.ual = CTRL.ual; //电机上面看到的电压就是控制器上的电压
    IM.ube = CTRL.ube;
}
int main(){
...
void write_data_to_file(FILE *fw){
    ...
    // if(CTRL.timebase>20)
    {
    if(++j == 10) //降采样的环节，每控制器执行10步，我只输出一步的数据
    {
        j=0;
        fprintf(fw, "%g,%g,%g,%g,%g\n", // fw -> file address ， 这里用的csv格式
